...
iPhone 個人熱點連 Windows 筆電，約 30-60 分鐘會自動斷線（iOS 省電機制已知問題）。此工具背景常駐，keep-alive ping 防斷線 + 斷線自動重連。
...
```powershell
# 手動執行
powershell -ExecutionPolicy Bypass -File scripts/wifi-keepalive.ps1
...
無。PowerShell + netsh，Windows 內建。

## 使用環境

僅 NB（HP Dragonfly, Windows 11, Intel Wi-Fi 6 AX200 160MHz）
