...
- **自訂 trigger**：`cl;`→`claude`、`tmls;`→`tmux ls`、`tmat;`→`tmux attach -t `、`tmne;`→`tmux new -s `、`tmre;`→`tmux rename-session -t `、`tmki;`→`tmux kill-session -t `、`cdjp;`→`cd ~/Projects`、`proj;`→`projects`。
...
- **跨機部署補充**：prd01／dev01 於同日完成 installer 與 `projects.yml` 重生。dev01 實測 launchd ancestry 直接讀 Dropbox File Provider 內容可能永久卡在 `open()`，連有 timeout 的 `shasum` 也只能反覆失敗；最終改用 metadata fingerprint（device／inode／size／mtime）偵測與防 TOCTOU，內容驗證交給有 timeout 的 Espanso CLI。
