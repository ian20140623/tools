...
- **跨機部署補充**：prd01／dev01 於同日完成 installer 與 `projects.yml` 重生。dev01 實測 Python 直接讀 Dropbox File Provider 可能永久卡在 `open()`；digest 改由有 20 秒 timeout 的 `shasum` 子程序執行，讓 background one-shot 能失敗退出並由下次事件重試。
